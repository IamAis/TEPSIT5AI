    ...
    static class PlayerHandler implements Runnable {
        ...
        public void run() {
            try (
            	...
            	Thread.sleep(300);
            	outgoingQueue.put(new DataTransfer("La partita è iniziata!"));
                if(First == true) {
                outgoingQueue.put(new DataTransfer("/EGOISTA")); 
                Thread.sleep(100);
                outgoingQueue.put(new DataTransfer("Inizia tu! Il tuo simbolo è O.")); 
                } else {
                outgoingQueue.put(new DataTransfer("/SECONDO"));
                Thread.sleep(100);
                outgoingQueue.put(new DataTransfer("Attendi il turno dell'avversario. Il tuo simbolo è X.")); 
                }
                ...
                while (true) {
                    ...
                    if (data.isChat()) {
                    	 if ("/PATE".equalsIgnoreCase(data.getMsg())) {
